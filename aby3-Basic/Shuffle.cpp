 ...
 * The shuffle protocol accoring to https://dl.acm.org/doi/10.1145/3460120.3484560 (advanced version).
*/
int efficient_shuffle_with_random_permutation(std::vector<sbMatrix> &T, int pIdx, std::vector<sbMatrix> &Tres, std::vector<aby3::si64> &Pi, aby3::Sh3Encryptor& enc, aby3::Sh3Evaluator& eval, aby3::Sh3Runtime& runtime){
    ...
    for(size_t i=0; i<len; i++){
        if(pIdx == 0) {
            Pi[i].mData[0] = ~0; Pi[i].mData[0] = i;
        }
        if(pIdx == 1) {
            Pi[i].mData[0] = ~0;  Pi[i].mData[1] = ~0;
        }
        if(pIdx == 2){
            Pi[i].mData[0] = i; Pi[i].mData[1] = ~0;
        }
    }
    ...
    std::vector<i64Matrix> prev_maskZ(len);
    std::vector<i64Matrix> next_maskZ(len);
    i64Matrix prev_maskRZ(len, T[0].bitCount());
    i64Matrix next_maskRZ(len, T[0].bitCount());
    ...
    if(pIdx == 0){
        ...
        i64Matrix maskRA(len, T[0].bitCount());
        ...
        i64Matrix sharedRY1_matrix(len, T[0].bitCount());
        runtime.mComm.mNext.recv(sharedRY1_matrix.data(), sharedRY1_matrix.size());
    ...
    if(pIdx == 1){
        ...
        i64Matrix maskRC(len, T[0].bitCount());
        ...
        i64Matrix sharedRX2_matrix(len, T[0].bitCount());
        runtime.mComm.mNext.recv(sharedRX2_matrix.data(), sharedRX2_matrix.size());
    ...
    if(pIdx == 2){
        ...
        // the following if for the shared permutation.
